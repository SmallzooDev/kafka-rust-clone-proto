use crate::adapters::incoming::protocol::constants::{
    ...
    FETCH_KEY, PRODUCE_KEY, UNKNOWN_TOPIC_OR_PARTITION,
    UNSUPPORTED_VERSION,
};
use crate::adapters::incoming::protocol::messages::{
    ApiVersionsResponse, DescribeTopicPartitionsResponse, ErrorCode, FetchResponse,
    FetchablePartitionResponse, FetchableTopicResponse, KafkaMessage, KafkaRequest,
    KafkaResponse, PartitionInfo, ProducePartitionResponse, ProduceResponse, ProduceTopicResponse,
    RequestHeader, RequestPayload, ResponsePayload,
...
impl KafkaBroker {
    ...
    async fn handle_produce_request(&self, request: &KafkaRequest, produce_request: &RequestPayload) -> Result<KafkaResponse> {
...
impl MessageHandler for KafkaBroker {
    async fn handle_request(&self, request: KafkaRequest) -> Result<KafkaResponse> {
        ...
        match request.header.api_key {
            ...
            PRODUCE_KEY => self.handle_produce_request(&request, &request.payload).await,
